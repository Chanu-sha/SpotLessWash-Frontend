...
const WalletWithdraw = ({ pickupOrders, deliveryOrders }) => {
  ...
  const fetchEarningsData = async () => {
    try {
      ...
      setEarningsData({
        ...
        todayPickupCount:
          Math.floor(
            todaysTotalCompletedOrders *
              (todayCompletedPickups.length /
                (todayCompletedPickups.length +
                  todayCompletedDeliveries.length))
          ) || todayCompletedPickups.length,
        todayDeliveryCount:
          Math.floor(
            todaysTotalCompletedOrders *
  ...
  const fetchWithdrawalHistory = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/deliveryboy/withdrawal-history`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleWithdrawal = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/deliveryboy/withdrawal-request`,
        {
          method: "POST",
          headers: {
  ...
  const EarningsModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full max-w-md mb-16 max-h-[85vh] overflow-hidden">
          {/* Header */}
                  ...
                  <>
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                      ...
                      <div className="flex justify-between text-sm text-green-700">
                        <span>
                          Total Orders: {earningsData.todaysTotalOrders || 0}
                        </span>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-center">
                        <div>
                          <div className="text-lg font-bold text-gray-800">
                            {earningsData.stats.completedPickups}
                          </div>
                        ...
                        <div>
                          <div className="text-lg font-bold text-gray-800">
                            {earningsData.stats.completedDeliveries}
                          </div>
                          <div className="text-xs text-gray-600">
                            Total Deliveries
                          </div>
                    ...
                    <button
                      ...
                      className="w-full mb-20 bg-purple-600 text-white py-3 rounded-md hover:bg-purple-700 transition-colors flex items-center justify-center space-x-2"
                    >
                      <FaHistory />
                    ...
                    <div className="grid grid-cols-1 gap-4 text-center">
                      <div>
                        <div className="text-2xl font-bold text-green-600">
                          {earningsData.todaysTotalOrders || 0}
                        </div>
                        <div className="text-xs text-gray-600">
  ...
  const WithdrawalModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full scale-[.97] mb-16 max-h-[90vh] max-w-md flex flex-col">
          {/* Header */}
          <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-4 rounded-t-lg text-white">
            <div className="flex justify-between items-center">
          ...
          {/* Footer Buttons */}
          <div className="p-4 border-t rounded-b-lg bg-white flex space-x-3 sticky bottom-0">
            <button
              type="button"
              onClick={() => setShowWithdrawModal(false)}
